...
for (let category in questionsData) {
    const uniqueQuestions = [];
    const seen = new Set();
    
    questionsData[category].forEach(item => {
        // Приводим вопрос к нижнему регистру и убираем лишние пробелы по краям
        const normalizedQ = item.q.trim().toLowerCase();
        
        // Если такого вопроса еще не было, добавляем в новый массив
    ...
    // Перезаписываем категорию массивом без точных дубликатов
    questionsData[category] = uniqueQuestions;
}
...
async function syncAndStartBlock(roomId, myNickname, isAdmin) {
    ...
    // Используем полноценный обработчик для ожидания данных
    const unsubscribe = onValue(indexesRef, (snap) => {
        const indexes = snap.val();
        
        if (indexes && Array.isArray(indexes) && indexes.length > 0) {
            unsubscribe(); // Немедленно отписываемся, чтобы избежать дублирования
...
function loadQuestion(category, roomId, myNickname) {
    ...
    grid.innerHTML = ""; // Очищаем сетку ПЕРЕД созданием новых кнопок
    ...
    answers.forEach((ans) => {
        const btn = document.createElement('button');
        btn.className = "answer-btn"; // Проверь наличие этого класса в CSS
