...
let timeLeft = 20;
let shuffledQuestions = []; // Массив для 15 синхронизированных вопросов

// Универсальная функция перемешивания (Фишер-Йетс)
...
async function checkVotes(roomId, votes) {
    ...
    if (votesCount >= playersCount) {
        ...
        await set(ref(database, `rooms/${roomId}/info/selectedCategory`), winner);
        await set(ref(database, `rooms/${roomId}/info/gameState`), "playing");
    }
}

// Синхронизированный запуск блока
async function syncAndStartBlock(roomId, myNickname, isAdmin) {
    ...
    if (isAdmin) {
        // Админ выбирает 15 рандомных индексов из доступных вопросов категории
        const allIndexes = Array.from(Array(questionsData[category].length).keys());
        const chosenIndexes = shuffle(allIndexes).slice(0, 15);
        // Сохраняем индексы в базу, чтобы у всех был один и тот же набор
        await set(ref(database, `rooms/${roomId}/info/currentQuestionIndexes`), chosenIndexes);
    }

    // Все игроки (включая админа) слушают появление индексов
...
function loadQuestion(category, roomId, myNickname) {
    ...
    answers.forEach((ans) => {
        ...
        btn.onclick = () => {
            ...
            if (!isCorrect) btn.style.background = "var(--error)";
